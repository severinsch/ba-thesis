...
We also analyzed some of the test cases provided by the Juliet Java test suite. The Test Suite is created by the National Security Agency’s (NSA) Center for Assured Software (CAS) and specifically designed to assess the capabilities of static analysis tools.
...
Since the Tricky example is artificially complex and the Juliet test cases have comparably low complexity, we created the example in Listing \ref{lst:sqlSanit}, which resembles a possible real attempt to escape an SQL input.
...
Figure \ref{fig:eval:plot:sqlSani} shows the execution time of analyzing the SQL query sanitization example from the previous section. Similar to the Tricky example, the plot shows the average of 100 measurements trimmed by 20\%. Again, note the different scale of the x axis compared to the previous examples. The difference compared to the other plots is explained with the different complexity of the analyzed code. Since the SQL query sanitization example is considerably less complex than the Tricky example, naturally the analysis execution time is lower. Similarly, for the Juliet examples smaller grammars and consequently smaller automata are created, and therefore the analysis is faster.

Note that the relative execution times of the different steps are fairly consistent among the different examples.
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[stackedBar,legend style={area legend, at={(0.5,-0.15)}, anchor=north, legend columns=3}]
			\addplot[fill=red!40] coordinates {(466.1666666666667, 4)};
			\addplot[fill=gray!40] coordinates {(168.28333333333333, 3)};
			\addplot[fill=purple!40] coordinates {(23.783333333333335, 2)};
			\addplot[fill=orange!40] coordinates {(166.75, 1)};
			\addplot[fill=blue!40] coordinates {(82.81666666666666, 0)};
			\legend{Grammar creation,Charset approximation,Regular approximation,Automaton 	creation,Regex creation}
		\end{axis}
	\end{tikzpicture}
