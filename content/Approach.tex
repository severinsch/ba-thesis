...
Note that for the algorithm an operation production of form $A \rightarrow op(X)$ is treated like an unary production of form $A \rightarrow X$. The operation productions are always handled by one of the loops in lines\ref{alg:left:firstrec}, \ref{alg:right:firstrec} or \ref{alg:nonrecloop}, because for any operation production contained in a cycle, the cycle is broken up by the character set approximation described in section \ref{charsetApprox}.
...
To eliminate a state $q$, we first replace each pair of edges $(q_0, r_1, q1), (q_0, r_2, q1)$ between two states with a single edge $(q_0, r_1|r_2, q_1)$ .

Now, we "shortcut" the state by replacing all pairs of transitions $(q',r, q), (q, t, q'')$ with a new transition from $q'$ to $q''$.
...
After repeatedly applying the two rules, the resulting automaton contains only the start and the end state. The single edge between those two states then has the resulting regular expression as a label.
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\path [-stealth, thick]
		(q0) edge node[above] {$r_1$} (q4)
		(q1) edge node[below] {$r_n$} (q4)
