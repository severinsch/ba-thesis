...
\begin{tikzpicture}[node distance=1.5cm, auto]
	    ...
	    \node [grammar, right=of CFG, xshift=15mm] (REG) {\ac{srg}};
...
To use the Mohri-Nederhof approximation algorithm \cite{mohri_nederhof} described in Section \ref{sec:mohriNederhofApprox}, we need to eliminate all cycles in our grammar that contain operation productions\cite{mohri_nederhof}.
...
To determine the \acp{scc}, we use Tarjan's algorithm \cite{tarjan}. This algorithm topologically sorts the returned components in reverse order, which is necessary for the fixpoint computation to terminate.
...
Mohri and Nederhof \cite{mohri_nederhof} describe an algorithm to transform a \ac{cfg} into a \ac{srg} as defined in Section \ref{sec:background:srg} that approximates the given \ac{cfg}.
...
Therefore, e.g. an operation production $C \rightarrow op(DX_1)$ with $C$ and $D$ in the same \ac{scc} can no longer occur.
